"""Registry + build pattern for class instantiation from config.

Configs are carried by OmegaConf ``DictConfig`` (or plain ``dict``).
Classes decorated with ``@register`` can be constructed by name via
``build(Base, cfg)``, where ``cfg["type"]`` selects the class from a
global registry and the remaining keys become constructor kwargs.
"""
...
def _is_config_dict(cfg: Any) -> bool:
    """True for plain ``dict`` and OmegaConf ``DictConfig`` (which is
    not a dict subclass but is the canonical config carrier)."""
...
def build(tp: type[T], cfg: dict | T | None, *args: object, **kwargs: object) -> T:
    ...
    if _is_config_dict(cfg):
        ...
        # Merge config values with kwargs (kwargs take precedence). Use
        # ``items()`` because both dict and DictConfig support it.
    ...
    assert isinstance(obj, tp), f"Type {type(obj)} did not match requested type {tp}"
